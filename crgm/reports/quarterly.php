...
<header class="navbar">
    ...
    <center><h1><strong>QUARTERLY REPORTS</strong></h1></center>
    ...
    <div class="row">
					...
					<a href="quarterly_print_pdf.php" class="btn btn-success pull-right"><span
							class="glyphicon glyphicon-print"></span> PDF</a>
					<a href="quarterly_export_excel.php" class="btn btn-success pull-right"><span
							class="glyphicon glyphicon-print"></span> Excel</a>
				</div>

                <br>

    <select id="selectQuarter" class="form-select form-control" onchange="filterData()">
        <option disabled selected>All Quarters</option>
...
?>
    ...
    <script>
        ...
        function filterData() {
            var selectedQuarter = document.getElementById("selectQuarter").value;

            $.ajax({
                type: "POST",
                url: "fetch_quarterly_data.php", // Replace with the actual filename where you put the PHP code
                data: { selectedQuarter: selectedQuarter },
                success: function (response) {
                    $("#quarterlyData").html(response);
                }
            });
        }
